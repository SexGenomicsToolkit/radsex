...
void table_parser(Parameters& parameters, Popmap& popmap, MarkersQueue& markers_queue, std::mutex& queue_mutex, bool& parsing_ended) {
    ...
    std::vector<Marker> tmp_queue(1000);  // Temporary block queue to avoid locking the shared blocks queue too often
        ...
        for (uint i=0; i<k; ++i) {
            ...
            switch(buffer[i]) {
                ...
                case '\t':  // New field
                    ...
                    switch (field_n) {
                        case 0:
                            ...
                            break;
                        case 1:
                            marker.sequence = temp;
                            break;
                        default:
                            marker.individuals[field_n - 2] = static_cast<uint>(std::stoi(temp));
                            if (static_cast<uint>(std::stoi(temp)) >= parameters.min_depth) ++marker.groups[groups[field_n]];
                            break;
                    }
                ...
                case '\n':  // New line (also a new field)
                    marker.individuals[field_n - 2] = static_cast<uint>(std::stoi(temp));
                    if (field_n > 1 and static_cast<uint>(std::stoi(temp)) >= parameters.min_depth) ++marker.groups[groups[field_n]];  // Increment the appropriate counter
                    // Add marker to the queue
                    tmp_queue[marker_n % 1000] = marker;  // Empty line means end of a block, we add it to the queue
                    ++marker_n;
                    if (marker_n % 1000 == 0 or field_n < 2) {  // Merge temporary queue with shared queue after 1000 blocks
                        queue_mutex.lock();
                        for (auto marker: tmp_queue) markers_queue.push(marker);
                        queue_mutex.unlock();
